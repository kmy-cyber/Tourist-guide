...
async def demo(): 
    ...
    # Datos de prueba con gaps
    test_data = {
        'name': 'Museo de la Revolución',
        'type': 'museum',
        'description': 'Museo en La Habana',
        'location': 'La Habana',
        # Faltan: schedule, price
        'last_updated': '2023-01-01T00:00:00'  # Desactualizado
    }
    
    integration = SimpleCrawlerIntegration()
    
    print("=== Datos Originales ===")
    print(json.dumps(test_data, indent=2, ensure_ascii=False))
    
    # Procesar con mejora automática
    result = await integration.process_query(
        "museo revolución habana horarios precios",
        test_data
    )
    
    print("\n=== Datos Mejorados ===")
    print(json.dumps(result['response'], indent=2, ensure_ascii=False))
    print(f"\nMejorado: {result['enhanced']}")
    ...
    print(f"Tasa de mejora: {stats['enhancement_rate']:.1%}")
